...
def load_xvg():															#DONE
	...
	global weights
	global distance
	global data_clustering_upper_POPC
	global data_clustering_upper_POPE
...
def calculate_avg():													#DONE
	...
	std_clustering_upper_POPC = np.sqrt(np.sum(weights) / float(np.sum(weights)**2 - np.sum(weights**2)) * tmp_upper_avg)	
